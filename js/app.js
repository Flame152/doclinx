...
window.addEventListener("resize", function () {
	if (document.documentElement.clientWidth < 1166) {
...
document.querySelectorAll('.main__video').forEach((el) => (
	el.addEventListener('click', () => {
		...
		if (/Firefox|Mac68K|MacPPC|MacIntel|Macintosh|macOS|iPhone|iPad|iPod|Android/i.test(navigator.userAgent)) {
			wrapperVideo.play();
		}
	})
));
...
const popupButton = document.querySelectorAll('.click-me, .services-block__image');
const body = document.body;
//Скачок у попапа
const popupWrapper = document.querySelectorAll('.popup-survey__wrapper, .popup-table__wrapper');
...
popupButton.forEach(btn => {
	btn.addEventListener('click', (e) => {
		...
		popups.forEach(popup => {
			if (popup.dataset.popup == data || popup.dataset.popup == e.target.closest('.click-me, .services-block__image').dataset.popupOpen) {
				openPopup(popup);
			}
		})
	})
...
function initSliders() {
	...
	//слайдер table
	if (document.querySelector('.table-mobile__slider')) { // Указываем скласс нужного слайдера
		// Создаем слайдер
		new Swiper('.table-mobile__slider', { // Указываем скласс нужного слайдера
			// Подключаем модули слайдера
			// для конкретного случая
			slidesPerView: 1,
			spaceBetween: 20,
			...
			// кнопки
			navigation: {
				nextEl: '.table-mobile__button-next',
				prevEl: '.table-mobile__button-prev',
			 },
		});
	}
}
